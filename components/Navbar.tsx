...
export default function Navbar() {
  ...
  return (
    <header className="sticky inset-x-0 top-0 z-[50]">
      <nav
        aria-label="Global"
        className="flex items-center justify-between px-4 sm:px-4 py-2.5 bg-gray-100 md:px-6 md:py-3 lg:px-7 lg:bg-white lg:bg-opacity-50 lg:backdrop-blur-[2px] shadow">
        <div className="flex lg:flex-1">
          <Link
            href="/"
            className="gap-1 sm:gap-2 text-center inline-flex items-center hover:bg-gray-300 hover:rounded-md px-2.5 py-1.5">
            ...
            <Image
              ...
              priority
            />
            <p className="font-figsemibold text-slate-800 text-lg sm:text-2xl lg:text-3xl">
              SB.
            </p>
          </Link>
        </div>
        ...
        <div className="hidden lg:flex lg:gap-x-1 lg:shrink-0">
          {navigation.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className={`text-md/6 text-slate-900 hover:shadow font-figregular tracking-wider hover:bg-gray-300 py-2.5 px-4 hover:rounded-md ${
                isActive(item.href) ? activeLinkStyles : ""
              }`}>
              {item.name}
            </a>
          ))}
      ...
      <Dialog
        ...
        <DialogPanel className="fixed inset-y-0 right-0 z-50 w-full sm:w-1/2 md:w-1/3 overflow-y-auto p-2 sm:p-4 bg-gray-50 shadow">
          <div className="flex items-center justify-between">
            <Link
              href="/"
              className="px-2.5 py-1.5 hover:bg-gray-300 rounded-md items-center">
              ...
              <div>
                {navigation.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    className={`block rounded-lg px-3 py-4 font-figsemibold text-gray-900 text-sm sm:text-base hover:bg-gray-200 ${
                      isActive(item.href) ? activeLinkStyles : ""
                    }`}>
                    {item.name}
                  </a>
